...
class Logging_rules_model extends CI_Model {
    ...
    private function _create_filter_string($filter) {
        ...
        if (is_array($filter)) {
            foreach ($filter as $field => $value) {
                if ($value) {
                    $parts[] = "lib_logging_rules.{$field} LIKE '%{$value}%'";
                }
            }
            $string = implode(' AND ', $parts);
            $string = (!empty($string) ) ? ' AND ' . $string : NULL;
        }
        return 'WHERE lib_logging_rules.old = 0 AND lib_logging_rules.provider_id = 0 ' . $string;
    }
    ...
    function get_keywords_count($filter) {
        ...
        $row = $this->_get_row(
                $this->db->query("
                SELECT COUNT( lib_logging_rules.id ) AS 'count'
                FROM lib_logging_rules;"
                )
        );
        ...
        if (is_array($data)) {
            foreach ($data as $id => $status) {
                $this->db_master->update(
                        'lib_logging_rules', array('basic' => $status), array('id' => $id), 1
                );
            }
        }
    }
        ...
        if (is_array($ids)) {
            foreach ($ids as $id) {
                $this->db_master->delete(
                        'lib_logging_rules', array('id' => $id), 1
                );
            }
        }
    }
    ...
    function add_keyword($data) {
        if (isset($data['keyword1'])) {
            $this->db_master->insert('lib_logging_rules', $data);
    ...
    function edit_keyword($id, $data) {
        if (isset($data['keyword1']) && !empty($id)) {
            ...
            $this->db_master->update('lib_logging_rules', $data);
            return true;
        }
        return false;
    }
