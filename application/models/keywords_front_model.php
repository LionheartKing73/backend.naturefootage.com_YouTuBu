...
class Keywords_front_model extends CI_Model {
    ...
    private function _create_filter_string($filter) {
        ...
        if (is_array($filter)) {
            foreach ($filter as $field => $value) {
                if ($value) {
                    $parts[] = "lib_keywords_front.{$field} LIKE '%{$value}%'";
                }
            }
            $string = implode(' AND ', $parts);
            $string = (!empty($string) ) ? ' AND ' . $string : NULL;
        }
        return 'WHERE lib_keywords_front.old = 0 AND lib_keywords_front.provider_id = 0 ' . $string;
    }
    ...
    function get_keywords_count($filter) {
        ...
        $row = $this->_get_row(
                $this->db->query("
                SELECT COUNT( lib_keywords_front.id ) AS 'count'
                FROM lib_keywords_front;"
                )
        );
   ...
   function get_frontends_list($filter = array(), $limit = array(), $order_by = ''){
        ...
        if (is_array($data)) {
            foreach ($data as $id => $status) {
                $this->db_master->update(
                        'lib_keywords_front', array('basic' => $status), array('id' => $id), 1
                );
            }
        }
    }
        ...
        if (is_array($ids)) {
            foreach ($ids as $id) {
                $this->db_master->delete(
                        'lib_keywords_front', array('id' => $id), 1
                );
            }
        }
    }
    ...
    function add_keyword($data) {
        if (isset($data['keyword'])) {
            $this->db_master->insert('lib_keywords_front', $data);
    ...
    function edit_keyword($id, $data) {
        if (isset($data['keyword']) && !empty($id)) {
            ...
            $this->db_master->update('lib_keywords_front', $data);
            return true;
        }
        return false;
    }
    ...
    function get_keywords_list($filter, $limit) {
        ...
        $result = $this->db->query("
            SELECT c.*,d.name
            FROM lib_keywords_front c LEFT JOIN lib_frontends d ON c.front_id = d.id
