...
class Upload extends CI_Controller {
    ...
    function index($id = 0, $type = 'res', $submission = '', $user_id = 0) {
		$target_dir = $this->config->item('clip_dir') . '/' . $type;
        ...
        if ($cleanupTargetDir && is_dir($target_dir) && ($dir = opendir($target_dir))) {
            while (($file = readdir($dir)) !== false) {
                $tmpfilePath = $target_dir . DIRECTORY_SEPARATOR . $file;
        ...
        if (!$chunks || $chunk == $chunks - 1) {
            if ($id) {
                $ext = $this->api->get_file_ext($filePath);
                $newPath = $target_dir . '/' . $id . '.' . $ext;
                
                $oldFile = $_SERVER['DOCUMENT_ROOT'] . '/'
                        . $this->clips_model->get_clip_path($id, $type);
                if (is_file($oldFile)) {
                    unlink($oldFile);
                }
                ...
                $this->clips_model->set_clip_res($id, $ext, $this->clips_model->res_type[$type]);
             } else {
                // Strip the temp .part suffix off 
                rename("{$filePath}.part", $filePath);
                $this->clips_model->create_clip($filePath, $submission, $user_id);
             }
        }


        // Return JSON-RPC response
        $this->output->set_output('{"jsonrpc" : "2.0", "result" : null, "id" : "id"}');
    }
