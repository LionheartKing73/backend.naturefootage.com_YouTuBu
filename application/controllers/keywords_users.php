 ...
 */
class Keywords_users extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model('keywords_users_model', 'km');
    ...
    function view() {
        ...
        $this->path = 'Library settings / Keywords Users';
        $paging = $this->api->get_pagination(
                'keywords_users/view', $this->km->get_keywords_count(
                        $this->_get_filter()
        ...
        $this->out(
                $this->load->view('keywords_users/view', $data, TRUE)
        );
    }
    ...
    function add() {
        $data = array(
            ...
            'section_id' => preg_replace('/<|>|\'|"|\(|\)/i', '', $this->input->post('section')),
            'collection_id' => preg_replace('/<|>|\'|"|\(|\)/i', '', $this->input->post('collection')),
            'provider_id' => preg_replace('/<|>|\'|"|\(|\)/i', '', $this->input->post('user_id')),
        );
        if (isset($data['keyword'])) {
            $this->km->add_keyword($data);
        }
        redirect('keywords_users/view');
    }
