...
<form action="<?=$lang?>/clips/galleries/<?=$id?>" method="post" class="form well">
    <fieldset>
        <legend>
            EDIT CLIP GALLERIES
        </legend>
        ...
        <table class="galleries">
            ...
            <?
            foreach($galleries as $val) { $i++;
                ?>

                <tr>
                    <td>
                        <input type="checkbox" <?if($val['checked']) echo "checked"?> name="id[]"
                               value="<?=$val['id']?>" id="gallery<?=$val['id']?>">
                    </td>
                    <td>
                        <label for="gallery<?=$val['id']?>"><?=$val['title']?></label>
                    </td>
                </tr>
