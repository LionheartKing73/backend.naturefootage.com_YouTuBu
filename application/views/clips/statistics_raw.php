...
<form name="filter" action="<?=$_SERVER['REQUEST_URI']?>" method="post" style="float: right; display: inline-block;" >
    <div class="toolbar-item <?php echo ($access>1)?'hide':''?>">
	    <div class="controls-group ">
		    <label>User:</label>
		    <input type="<?php echo ($access>1)?'hidden':'text'?>" name="user" value="<?=$user?>" style="width: 120px">
	    </div>
	    <div class="controls-group">
		    <input type="submit" name="userfilter" value="Filter" class="btn find">
	    </div>
    </div>
    <br class="clr">
</form>
...
<table border="0" cellpadding="1" cellspacing="1" >
	...
	<tr class="table_title" style="font-weight: bold; border-bottom: 4px double #ccc;">
		<th style="padding: 5px 10px;width: 140px;" data-order="stat.action_type"><?=$this->lang->line('action');?> <?php echo sortByArrow('stat.action_type'); ?></th>
		<th style="padding: 5px 10px;width: 140px;" data-order="c.code">Clip <?php echo sortByArrow('c.code'); ?></th>
		<th style="padding: 5px 10px;width: 140px;" data-order="stat.user_login" class="<?php echo ($access>1)?'hide':''?>">User <?php echo sortByArrow('stat.user_login'); ?></th>
		<th style="padding: 5px 10px;width: 140px;" data-order="stat.time"><?=$this->lang->line('date');?> <?php echo sortByArrow('stat.time'); ?></div></th>
	</tr>
    </thead>
    <tbody>
	<?php if( $statistic ): foreach( $statistic as $log ): ?>
		<tr class="tdata1">
			<td style="padding:3px;" onmouseover='light(this);' onmouseout='dark(this);' >
                <?php switch ($log[ 'action_type' ]){
                    case 1: echo 'Clip Viewed'; break;
                    case 2: echo 'Preview Downloaded'; break;
                    case 3: echo 'Clip Licensed'; break;
                } ?>
            </td>
			<td style="padding:3px;" ><?= $log[ 'code' ] ?></td>
			<td style="padding:3px;" class="<?php echo ($access>1)?'hide':''?>"><?= $log[ 'user_login' ] ?></td>
