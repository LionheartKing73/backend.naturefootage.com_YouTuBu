...
<!--form name="filter" action="<?=$lang?>/clips/statistics<?='/all';/*.$id*/?>" method="post" style="float: right; display: inline-block;" >
...
<table border="0" cellpadding="1" cellspacing="1" >
	...
	<tr class="table_title" style="font-weight: bold; border-bottom: 4px double #ccc;">
		<th style="padding: 5px 10px;width: 140px;" data-order="stat.user_login"><?=$this->lang->line('users');?> <?php echo sortByArrow('stat.user_login'); ?></th>
		<th style="padding: 5px 10px;width: 140px;" data-order="count"><?=$this->lang->line('downloads');?> <?php echo sortByArrow('count'); ?></th>
	</tr>
    </thead>
    <tbody>
	<?php if( $statistic ): foreach( $statistic as $log ): ?>
		<tr class="tdata1" data-url="<?=$lang?>/clips/statistics/all/user/<?=$log['user_login']?>">
			<td style="padding:3px;" onmouseover='light(this);' onmouseout='dark(this);' ><?= $log[ 'user_login' ] ?></td>
			<td style="padding:3px;" ><?= $log[ 'count' ] ?></td>
		</tr>
	<?php endforeach; else: ?>
