...
if (count($cats)) {
    ...
    foreach($cats as $cat) {
        ++$num;
        ?>
    <div class="result<?if(!($num%4)){?> last<?}?>">
        <div class="top_bar">
            <a href="<?=$lang.$action.'/'.$cat['uri'].'.html'?>"><?=$cat['title']?></a>
        </div>
        <div>
            <a href="<?=$lang.$action.'/'.$cat['uri'].'.html'?>"><img src="<?=$cat['thumb']?>" width="200" alt="<?=$cat['alt'] ? $cat['alt'] : $cat['title']?>"></a>
        </div>
