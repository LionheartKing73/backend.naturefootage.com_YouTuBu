...
class Parser
{
    ...
    public function parse($string)
    {
        ...
        // remove words which length <= $this->shortWordsLength()
        if ($this->shouldRemoveShortWords()) {
            $tokens = $this->removeShortWords($tokens);
        }
        // --
        ...
        $lasttoken = "";
        foreach ($tokens as &$ktoken) {
            //No word stemming or wildcard when word in quotes
            if ( $ktoken[strlen($ktoken) - 1] != "*" && strpos($ktoken, "\"") === false && strpos($ktoken, "-") === false && strpos($ktoken, "+") === false) {
                //Word Stemming (es and s only);
                // No word stemming after NOT
                if (strpos($lasttoken, self::NOT_TOKEN_CHARACTER) === false && strlen($ktoken) > 3) {
                    if ($this->endsWith($ktoken, "es")) {
                        $ktoken = substr($ktoken, 0, strlen($ktoken) - 2);
                    } else if ($this->endsWith($ktoken, "s")) {
                        $ktoken = substr($ktoken, 0, strlen($ktoken) - 1);
                    }
                }
                //Add wildcard to all work by demand of Dan
                if(trim($ktoken) != self::OR_TOKEN_CHARACTER) {
                    $ktoken = $ktoken . "*";
                }
            }
            $lasttoken = $ktoken;
    ...
    function removeCommonWords($input){
        $commonWords = array("a","about","an","are","as","at","be","by","com","de","en","for","from","how","i","in","is","it","la","of","on","that","the","this","to","was","what","when","where","who","will","with","und","the","www");
    ...
    private function secondClean($tokens)
    {
        ...
        foreach ($tokens as $token) {
            $token = $string = preg_replace('/[^A-Za-z0-9\x00-\x1F\x80-\xFF @\(\)\-\+\*\"\'\.]/', '', $token);
            $toReturn[] = $token;
        }
    ...
    /**
     * determine if short words should be removed from searc term
     *
     * @return bool
     */
    protected function shouldRemoveShortWords()
    {
        return true;
    }

    /**
     * all words with length shorter or equal this value will be ignored on final search string
     *
     * @return int
     */
    protected function shortWordsLength()
    {
        return 2;
    }

    /**
     * any words which length is shorter or equal than $this->shortWordsLength will be removed from $tokens array
     *
     * @param array $tokens - array to filter
     *
     * @return array filtered $tokens array
     */
    private function removeShortWords(array $tokens)
    {
        $skippedWords = [
            self::AND_TOKEN,
            self::OR_TOKEN,
            self::NOT_TOKEN,
            self::AND_TOKEN_CHARACTER,
            self::OR_TOKEN_CHARACTER,
            self::NOT_TOKEN_CHARACTER,
            self::LEFT_BRACKET_TOKEN_CHARACTER,
            self::RIGHT_BRACKET_TOKEN_CHARACTER
        ];

        $arrayResults = array_filter($tokens, function($item) use ($skippedWords) {
            return in_array($item, $skippedWords) || strlen($item) > $this->shortWordsLength();
        });
        return array_values($arrayResults);
    }
}