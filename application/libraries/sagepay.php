...
class Sagepay {
  ...
  var $strYourSiteFQDN;
  var $strVendorName;
  var $strEncryptionPassword;
  var $strVendorEMail;
  var $strCurrency;
  var $strTransactionType = 'PAYMENT';
  var $strPartnerID = '';
  var $bSendEMail = 1;
  ...
  function Sagepay() {
    ...
    $this->strConnectTo = $this->CI->config->item('Sagepay_ConnectTo');
    $this->strVirtualDir = $this->CI->config->item('Sagepay_VirtualDir');
    $this->strYourSiteFQDN = $this->CI->config->item('Sagepay_YourSiteFQDN');
    $this->strVendorName = $this->CI->config->item('Sagepay_VendorName');
    $this->strEncryptionPassword = $this->CI->config->item('Sagepay_EncryptionPassword');
    $this->strVendorEMail = $this->CI->config->item('Sagepay_VendorEMail');
    
    if ($this->strConnectTo == 'LIVE') {
    	$this->strPurchaseURL = 'https://live.sagepay.com/gateway/service/vspform-register.vsp';
    }
    elseif ($this->strConnectTo == 'TEST') {
    	$this->strPurchaseURL = 'https://test.sagepay.com/gateway/service/vspform-register.vsp';
    }
    else {
    	$this->strPurchaseURL = 'https://test.sagepay.com/simulator/vspformgateway.asp';
    }
  }
  ...
  ** Works even if one of the values is a URL containing the & or = signs.                                      	  */
  ...
  function cleanInput($strRawText, $filterType)
  {
      ...
      {
          $strAllowableChars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 .,'/\\{}@():?-_&Ј$=%~*+\"\n\r";
  ...
  **  more secure than the other PSPs who don't both encrypting their forms at all                                      */
  ...
  function encryptAndEncode($strIn) {
  	...
  	else
  	{
  		...
  		//** use initialization vector (IV) set from $this->strEncryptionPassword
  ...
  function decodeAndDecrypt($strIn) {
  	...
  	else
  	{
  		...
  		return $this->simpleXor($this->base64Decode($strIn),$this->strEncryptionPassword);
  	}
  }
