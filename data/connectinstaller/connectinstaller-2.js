	...
	// ConnectInstaller
		...
		// AW.utils.logger("ConnectInstaller.internalInstallLatest - Aspera installer update starting");
				...
				that.awInstaller.setCallback(function(state, description, percent) {
					...
					if (state == "COMPLETE") {
						...
						connectInstaller.internalInstallLatest(); // Continue with original execution
					} else {
	  ...
	  that.awInstaller.executeCommands(commandList); // Async
		...
		this.isUpgrade = function () {	
		  if (connectInstaller.installedVersion && connectInstaller.updateAvailable()) {
	...
	function getConnectReleaseVersion (pluginEl) {
	  ...
	  return version;
		...
		this.init = function(options) {
			...
			options.minVersion = options.minVersion || that.availableVersion;
			
			this.minVersion = options.minVersion;
			
			if (connectInstaller.installedVersion && 
				!AW.utils.versionLessThan(connectInstaller.installedVersion, options.minVersion) &&
				options.connectReady) 
			{
			...
			if (connectInstaller.installedVersion && // The installedVersion could be undefined and versionLessThan can't deal.
				AW.utils.versionLessThan(connectInstaller.installedVersion, options.minVersion) && 
				options.install) 
			{
			...
			if (!connectInstaller.installedVersion && options.install) {
		...
		function parseNavigator() {
		  ...
		  // in connectversions.js.
			...
			if (agent.indexOf("Windows NT 6.2") != -1) that.os = "Win32"; //Win8
		...
		this.getEnvSupportInfo = function () {
			...
			envProfiles = {
				...
				win8Ie10_64bit : {
					// Plugins are broken in IE10 64-bit tile app.
					test : (AW.utils.browser.is.ie && /Windows NT 6.2/.test(uAgent) && /MSIE 10/.test(uAgent) && /Win64/.test(uPlatform)),
