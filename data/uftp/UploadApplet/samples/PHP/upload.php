...
$username = $_POST["username"]; //from the login form
$password = $_POST["password"]; //from the login form
$key = "12345"; //enter any random value as the key

$server = "roon.doctera.com"; //your FTP server
...
function encode($sstring, $skey)
{
    ...
    for ($i = 0; $i < $slen; $i++){
      ...
      $newval = $val + ord($skey{$i%$klen});
      while ($newval<=32||$newval>126||$newval==34||$newval==39)
        $newval=($newval<=32)?126- abs(32-$newval):($newval>126)?$newval-94:$newval+ord($skey{$i%$klen});

      $sstring{$i} = chr($newval);
    }

    return $sstring;
}

$user = encode($username, $key);
$pass = encode($password, $key);
$server = encode($server, $key);
